...
void lcd_print_int(int num);                                                // Prints an integer on the lcd
...
/***********************************************************
lcd_print_int(<integer value>):
	Converts an integer to its ASCII characters and 
	prints them on the LCD. Handles negative numbers.
***********************************************************/	
void lcd_print_int(int num)
{
	unsigned char buf[6]; // Buffer to hold digits (max 16-bit int is 32767)
	int i = 0;
	...
	// Extract digits (Note: This extracts them in reverse order)
	while (num > 0)
	{
	...
	// Print the characters in reverse to get the correct order
	while (i > 0)
	{
		i--;
